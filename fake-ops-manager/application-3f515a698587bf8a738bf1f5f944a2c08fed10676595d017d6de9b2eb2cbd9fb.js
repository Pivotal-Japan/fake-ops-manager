 ...
 *
 *
 * Edited by OpsMan team to fix data on connection closure bug.  Search for "Added by OpsMan Team"
 *
 *
 */
		...
		xhr: function(stream, handler, message) {
			...
			xhr.onreadystatechange = function() {
				...
				// Handles error or close event
					...
					// Added by OpsMan Team to fix bug where data isn't returned as the connection gets closed.
					if (xhr.status === 200) {
						handler.response(xhr.responseText);
					}
...
$(document).ready(function() {
  $(document).on("click.rails", "a.show-loading-indicator", function() {
...
// For jshint:
/* global AccordionGroup: false */


window.AccordionGroup = {
  setup: function() {
    $('.accordion-group').each(function() {
      var $group = $(this);
      $group.find('.accordion-group-section').hide();

      $group.find('.accordion-group-radio').click(function() {
        var $radio = $(this);
        $group.find('.accordion-group-section').hide();
        if ($radio.attr('id')) {
          $group.find('[data-accordion-target-of-id="' + $radio.attr('id') + '"]').show();
        }
      });

      var $checkedRadio = $group.find('.accordion-group-radio:checked');
      if ($checkedRadio.attr('id')) {
        $group.find('[data-accordion-target-of-id="' + $checkedRadio.attr('id') + '"]').show();
      }
    });
  }
};
...
function adjustLayout(){
  ...
  if ($(".flash-message").is(':visible')){
    ...
    $(".flash-message:visible").each(function(){
      flash_message_height += $(this).outerHeight();
    });
  }
...
function AutoScroll($el) {
  ...
  $el.scroll(function(e) {
    ...
    if (autoScrollTriggeredThisScroll) {
      ...
      var contentHeightEvenIfHidden = $el.prop('scrollHeight');
  ...
  function scrollToBottom() {
    ...
    scrollTimeout = window.setTimeout(function() {
      $el.animate({scrollTop: $el.prop('scrollHeight')}, 0.1);
    }, 100);
  }
...
;
// For jshint:
/* global flashMessage: false */


var AvailableProducts = {
  setup: function() {
    AvailableProducts.pivnetEndpoints = $('#pivnet-endpoints').data();

    // Handle expand / collapse of version list sections
    $('.collapse-product-versions .title').click(function(e) {
      var $scope = $(e.target).closest('.product');
      $scope.find('.collapsible-product-version').toggle();
      $(e.target).toggleClass('expanded');
    });

    // Handle download button click
    $('.download-from-pivnet').click(function() {
      var $downloadButton = $(this);
      AvailableProducts.lockDownloadButtons($downloadButton);

      AvailableProducts.downloadFromPivNet({
        product_name: $downloadButton.data('identifier'),
        version: $downloadButton.data('version')
      }, $downloadButton);
    });

    $('.download-progress-display:visible').each(function(i, downloadProgressDisplay) {
      var $downloadProgressDisplay = $(downloadProgressDisplay);
      AvailableProducts.trackDownloadProgress($downloadProgressDisplay.data('download-id'), $downloadProgressDisplay);
    });

    // Handle modal accept
    $('#accept-pivnet-eula.modal #accept-eula-button').click(function() {
      AvailableProducts.acceptEula($('#eula-content').data('params'), $('#eula-content').data('download-button'));
    });

    // Handle modal cancel
    $('#accept-pivnet-eula.modal').on('hidden.bs.modal', function() {
      AvailableProducts.unlockDownloadButtons($('#eula-content').data('download-button'));
    });
  },

  downloadFromPivNet: function(payload, $downloadButton) {
    $.post(AvailableProducts.pivnetEndpoints.download, payload).success(function(response) {
      flashMessage('success', 'Download has been queued.');
      AvailableProducts.unlockDownloadButtons($downloadButton);

      var $downloadProgressDisplay = AvailableProducts.replaceDownloadButtonWithProgressDisplay($downloadButton);
      AvailableProducts.updateDownloadProgress({total_bytes: 0}, $downloadProgressDisplay);
      AvailableProducts.trackDownloadProgress(response.download_id, $downloadProgressDisplay);

    }).error(function(response) {
      if (response.responseJSON && response.responseJSON.has_eula_been_accepted === false) {
        AvailableProducts.showEulaForAcceptance(payload, $downloadButton);
      } else {
        AvailableProducts.handleError(response, $downloadButton);
      }
    });
  },

  replaceDownloadButtonWithProgressDisplay: function($downloadButton) {
    var $downloadProgressDisplay = $('<div>');

    $downloadButton.before($downloadProgressDisplay);
    $downloadButton.hide();

    return $downloadProgressDisplay;
  },

  replaceProgressDisplayWithDownloadButton: function($downloadProgressDisplay) {
    var $downloadButton = $downloadProgressDisplay.next('.download-from-pivnet');
    $downloadProgressDisplay.hide();
    $downloadButton.show();
  },

  trackDownloadProgress: function(downloadId, $downloadProgressDisplay) {
    $.get(AvailableProducts.pivnetEndpoints.download + "/" + downloadId).success(function(response) {
      var downloadStatus = response.download.status;

      var queueNextCheck = function() {
        AvailableProducts.trackDownloadProgress(downloadId, $downloadProgressDisplay);
      };

      if (['QUEUED', 'DOWNLOADING'].indexOf(downloadStatus) >= 0) {
        AvailableProducts.updateDownloadProgress(response.download, $downloadProgressDisplay);
        setTimeout(queueNextCheck, 3000);

      } else if (downloadStatus == 'IMPORTING') {
        $downloadProgressDisplay.text('Importing');
        setTimeout(queueNextCheck, 3000);

      } else if (downloadStatus == 'COMPLETED') {
        flashMessage('success', "Download complete for " + response.download.product_identifier + " " + response.download.product_version + ". Please refresh the page to use the product.");
        $downloadProgressDisplay.text('Completed');

      } else if (downloadStatus == 'FAILED') {
        flashMessage('error', response.download.error_message);
        AvailableProducts.replaceProgressDisplayWithDownloadButton($downloadProgressDisplay);
      }
  ...
  updateDownloadProgress: function(download, $downloadProgressDisplay) {
    var progress = 0;

    if (download.total_bytes) {
      progress = Math.round(download.bytes_downloaded / download.total_bytes * 100);
    }

    $downloadProgressDisplay.text(progress + '%');
  },

  showEulaForAcceptance: function(params, $downloadButton) {
    $.get(AvailableProducts.pivnetEndpoints.eula, params).success(function(response) {
      $('#eula-content').data({params: params, 'download-button': $downloadButton}).html(response.eula);
      $('#accept-pivnet-eula.modal').modal('show');
    }).error(function(response) {
      AvailableProducts.handleError(response, $downloadButton);
    });
  },

  acceptEula: function(payload, $downloadButton) {
    var data = {
      method: 'PUT',
      data: _({accept: true}).extend(payload)
    };

    $.ajax(AvailableProducts.pivnetEndpoints.eula, data).success(function() {
      AvailableProducts.downloadFromPivNet(payload, $downloadButton);
  ...
  lockDownloadButtons: function($clickedButton) {
    $('.download-from-pivnet').css('pointer-events', 'none');
    $clickedButton.find('i.fa').removeClass('fa-download').addClass('fa-spinner fa-spin');
  },

  unlockDownloadButtons: function($clickedButton) {
    $('.download-from-pivnet').css('pointer-events', 'auto');
    $clickedButton.find('i.fa').removeClass('fa-spinner fa-spin').addClass('fa-download');
  },

  handleError: function(response, $downloadButton) {
    flashMessage('error', AvailableProducts.getFormattedErrorsMessage(response.responseJSON.errors));
    AvailableProducts.unlockDownloadButtons($downloadButton);
  },

  getFormattedErrorsMessage: function(errorsResponse) {
    var messages = [];
    for (var key in errorsResponse) {
      messages.push(key + ": [" + errorsResponse[key].join(', ') + "]");
    }
...
$(function() {
  AvailableProducts.setup();
});
// For jshint:
/* global ChangeLogDataTable: true */
/* global moment: false */


window.ChangeLogDataTable = {
  presenter: function(item) { return _.escape(item.label + " " + item.product_version); },

  dateFormatter: function(date) { return moment.utc(date).format('YYYY-MM-DD HH:mm:ss') + ' UTC'; },
  ...
  config: function() {
    return {
      ...
      "paging": true,
      "pageLength": 25,
      "ajax": { "url": "/api/v0/installations.json", "dataSrc": "installations" },
      "language": {
        "emptyTable": "No installations have been run"
      },
      "columns": [
        { "data": function(data) { return data.started_at && ChangeLogDataTable.dateFormatter(data.started_at); } },
        { "data": function(data) { return data.finished_at && ChangeLogDataTable.dateFormatter(data.finished_at); } },
        { "data": "user_name" },
        { "data": function(data) { return data.status.toUpperCase(); } },
        {
          "data": function(data) {
            return _.map(data.additions, ChangeLogDataTable.presenter).join("<br>");
          }
        },
        {
          "data": function(data) {
            return _.map(data.updates, ChangeLogDataTable.presenter).join("<br>");
          }
        },
        {
          "data": function(data) {
            return _.map(data.deletions, ChangeLogDataTable.presenter).join("<br>");
          }
        },
        { "data": function(data) {
            return "<a href='/installation_logs/" + data.id + "'>Logs</a>";
          }
        }
      ]
    };
...
function bindCheckMultiple($el) {
  $el.on("change", ".check-multiple:checkbox", function () {
    var checked = $(this).prop("checked");
    var networkName = $(this).data("network-name");
    var selector = ":checkbox[name*='["+networkName+"]']";
    var checkboxes = $el.find(selector);
...
$(function() {
  var $deleteButton = $("#delete_unused_products");
  var $confirmDialog = $('#confirm-delete-unused-products');
  ...
  $confirmDialog.on('show.bs.modal', function() {
    var modal = $(this);
    var openerData = modal.data('openerData');
    var productName = openerData.productname;
    var version = openerData.version;

    if (productName) {
      modal.find('.unused-products-to-be-deleted').text(productName + ' (' + version + ')');
      modal.find('#delete_unused_products').data('url', '/api/v0/available_products?product_name=' + productName + '&version=' + version);
    } else {
      modal.find('.unused-products-to-be-deleted').text(modal.data('all-unused-product-names'));
      modal.find('#delete_unused_products').data('url', '/api/v0/available_products');
    }
  });

  // Our version of Bootstrap does not set event.relatedTarget, so we pass in the data attributes manually via a data call.
  $('.open-delete-products-modal').on('click', function(e) {
    e.preventDefault();
    $confirmDialog.data('openerData', $(this).data());
    $confirmDialog.modal('show');
  });
});
$(function() {
  var fieldSelector = "input[type='password'], input[type='text'], textarea";

  // Handle fields present at load.
  $(fieldSelector).attr('autocomplete', 'off');

  // Handle fields created in JS.
  $(document).on('focus', fieldSelector, function(){
    $(this).attr('autocomplete', 'off');
  });
});
// For jshint:
/* global adjustLayout: false */
...
function Export($el, loadingIndicator) {
  var obj = {
    bindClick: function() {
      $el.on("click", function() {
        loadingIndicator.show();

        $.post($el.attr("href")).success(function(){
          obj.redirectTo($el.data("download-path"));
        }).complete(function() {
          loadingIndicator.hide();
        }).error(function() {
          obj.redirectTo("/500.html");
        });
        return false;
      });
    },
...
$(function() {
  $(".prepare-export").each(function() {
    new Export($(this), new LoadingIndicator());
  });
});
// For jshint:
/* global adjustLayout: false */
/* exported flashMessage */


function flashMessage(type, message) {
  $('.flash-message').remove();
  var $flashMessage = $('<div>').addClass('flash-message').addClass(type).text(message);
  $('.site-header').after($flashMessage);
  adjustLayout();
}
;
$(window).load(function() {
  setTimeout(function() {
    var $input = $("form input:text:not(:disabled)").first();
    $input.focus();
  }, 0);
});
function bindIgnoreDisabled($el) {
  $el.on("submit", function () {
    $(this).find(":input.ignore-disabled").removeProp("disabled");
  });
}

$(function () {
  bindIgnoreDisabled($("form"));
});
var MaskedFieldWithChangeLink = {
  setup: function ($input) {
    $input.val('********');
    $input.prop('disabled', true);
    $input.attr('class', 'disabled');

    var name = $input.attr('name');
    var $changeLink = $('<a href="#">Change</a>').attr('data-masked-input-name', name);
    var $cancelLink = $('<a href="#">Cancel</a>').attr('data-masked-input-name', name);

    $changeLink.click(function (e) {
      $input.prop('disabled', false);
      $input.val('');
      $input.removeClass('disabled');
      $changeLink.after($cancelLink);
      $changeLink.detach();

      MaskedFieldWithChangeLink.showRsaCertLinkIfExists($input);
    ...
    $cancelLink.click(function (e) {
      ...
      $input.attr('class', 'disabled');
      $input.val('********');
      $cancelLink.after($changeLink);
      $cancelLink.detach();

      MaskedFieldWithChangeLink.hideRsaCertLinkIfExists($input);

      e.preventDefault();
    });

    if ($input.parent('.controls').length > 0) {
      $changeLink.insertAfter($input.parent());
    } else {
      $changeLink.insertAfter($input);
    }
    MaskedFieldWithChangeLink.hideRsaCertLinkIfExists($input);
    return {
  ...
  applyToDOM: function () {
    $('[data-masked-field-with-change-link="true"][data-bosh-credential]:not(:disabled)').each(function () {
      MaskedFieldWithChangeLink.setup($(this));
    });
  },

  showRsaCertLinkIfExists: function (input) {
    var $rsaCertGeneration = input.parent('.controls').children('.with-rsa-certificate-link');
    if ($rsaCertGeneration.length > 0) {
      $rsaCertGeneration.show();
    }
  },

  hideRsaCertLinkIfExists: function (input) {
    var $rsaCertGeneration = input.parent('.controls').children('.with-rsa-certificate-link');
    if ($rsaCertGeneration.length > 0) {
      $rsaCertGeneration.hide();
    }
  }
};

var MaskedFieldWithChangeLinkForProductForms = {
  setup: function ($input) {
    var $links = MaskedFieldWithChangeLink.setup($input);

    var $hiddenInput = $('<input type="hidden" value="true" />');
    var name = $input.attr('name');
    $hiddenInput.attr('name', name + '[unchanged]');
    $input.before($hiddenInput);

    if ($input.is('[readonly]')) {
      $links.changeLink.hide();
    }

    $links.changeLink.click(function () {
      $hiddenInput.prop('disabled', true);
    });

    $links.cancelLink.click(function () {
      $hiddenInput.prop('disabled', false);
    });
  },

  applyToDOM: function () {
    $('[data-masked-field-with-change-link="true"][data-product-credential]:not(:disabled)').each(function () {
      MaskedFieldWithChangeLinkForProductForms.setup($(this));
    });
  }
};

$(function () {
  MaskedFieldWithChangeLink.applyToDOM();
  MaskedFieldWithChangeLinkForProductForms.applyToDOM();
});
function bindReverseHereditaryFocus($wrapper, ancestorSelector) {
  var childSelector = ancestorSelector+" *";
  var oldAncestor = null;

  $wrapper.on("mouseover focusin", childSelector, function () {
    var $ancestor = $(this).parents(ancestorSelector);

    if (oldAncestor !== null && oldAncestor != $ancestor) {
      oldAncestor.removeClass("focused");
    }
    $ancestor.addClass("focused");
...
$(function () {
  bindReverseHereditaryFocus($("body"), ".reverse-hereditary-focus");
});
...
$(function(){
  $(".submit-on-change").each(function(){
    new SubmitOnChange($(this));
  });
});


var SubmitOnChange = function($el){
  $el.change(function(){
    $(this).parents("form").submit();
  });
};
var ToggleDisabledFields = function($container) {
  $container.on('change', ':checkbox', function() {
    toggleEnabledAndDisabled();
  });
  ...
  function toggleEnabledAndDisabled() {
    $container.find('[data-fields-enabled-by]').each(function() {
      var $radioButton = $container.find('#' + $(this).data('fields-enabled-by'));
      if($radioButton.is(':checked')) {
        $(this).removeClass('disabled');
        $(this).find(':input:not([data-masked-field-with-change-link=true])').prop('disabled', false);
      }
      else {
        $(this).addClass('disabled');
        $(this).find(':input:not([data-masked-field-with-change-link=true])').prop('disabled', true);
      }
    });
  }
...
ToggleDisabledFields.hookOntoToggleDisabledFields = function() {
  $('[data-toggle-disabled-fields]').each(function() {
    new ToggleDisabledFields($(this));
  });
};

$(function() {
  ToggleDisabledFields.hookOntoToggleDisabledFields();
});
$(function(){
...
function bindToggleExpandable($el) {
  $el.on("click", ".expandable .title", function () {
    $(this).closest(".expandable").toggleClass("expanded");
  });
}

$(function () {
  bindToggleExpandable($("body"));
});
$(function() {
...
$(function() {
  ...
  $('.truncate-text')
    ...
    .find("p").click(function() {
      ...
      if ($this[0].offsetWidth < $this[0].scrollWidth && $(".show-truncated-text").length === 0)
        $this.prepend("<h3 class='show-truncated-text'>"+$this.text()+"</h3>");
  ...
  $(document).click(function(event) {
    ...
    if ( !! $(target).is(".show-truncated-text") || $(target).find(".show-truncated-text").length === 0 || $(".show-truncated-text").length === 0)  {
      $('.show-truncated-text').remove();
    }
  });
...
function BackgroundTask($el, setupMethod, setupUrl, includeAll) {
  ...
  self.onTrackMessage = function(event) {
...
function CopiesTemplate(args) {
  var template = args.template;
  var appendTo = args.appendTo;
  var clickElement = args.clickElement;
  var indexReplacementString = args.indexReplacementString;
  var index = args.startIndex || 0;

  clickElement.click(function (event) {
    var appendMe;

    if (indexReplacementString){
      appendMe = template.html().replace(new RegExp(indexReplacementString, 'g'), index);
    } else {
      appendMe = template.html();
    }

    index = index + 1;

    appendTo.append(appendMe);
    appendTo.trigger("record-added-event");
    event.preventDefault();
  });
}

$(function() {
  $(".with-copies-template").each(function(){
    var $clickElement = $(this);

    new CopiesTemplate({
      template: $("#" + $clickElement.data("templateId")),
      appendTo: $("#" + $clickElement.data("appendTo")),
      clickElement: $clickElement,
      indexReplacementString: $clickElement.data("indexReplacementString"),
      startIndex: $clickElement.data("startIndex")
    });
  });
});
$(function() {
  var selector = "select.with-custom-textbox-replacement";
  ...
  $(document).on('change', selector, function() {
    ...
    if ($select.val() === 'custom') {
      var $input = $("<input type='text'>")
        ...
        .val($select.data('previous-value'));
...
$(document).on("click", ".with-delete-record", function() {
...
$(document).on('click', '.with-remote-content', function(event) {
  var $clickElement = $(event.target);
  var index = $clickElement.data('remoteCollectionIndex');
  $clickElement.data('remoteCollectionIndex', index + 1);
  var $appendTo = $($clickElement.data("appendTo"));
  var url = $clickElement.data("url");
...
$(document).ready(function(){
  $(".with-rsa-certificate-link").click(function() {
...
$(document).ready(function(){
  $("form.show-loading-indicator").each(function() {
    var saveLoading = new SaveLoading($(this));
    saveLoading.reactToSubmit();
    saveLoading.reactToError();
  });
});
function StatusData($table) {
  var self = {};

  var $instanceRows = $table.find("[data-job-name]");
  ...
  self.apply = function(response) {
    ...
    if (response.status) {
      var statusMap = buildStatusMap(response.status);
      $instanceRows.each(function() {
        new InstanceWithStatus($(this), statusMap, $table).updateHtml();
      });
    }
    else {
      $instanceRows.each(function() {
        new InstanceWithError($(this), response.error).updateHtml();
      });
    }
  };

  function buildStatusMap(instanceStatuses) {
    var statusMap = _.groupBy(instanceStatuses, function(instanceInfo) {
      return instanceInfo.job_name;
    });

    _.each(statusMap, function(instanceInfos, jobName) {
      statusMap[jobName] = _.sortBy(instanceInfos, function(instanceInfo) {
...
function InstanceWithStatus($row, statusMap, $table) {
  var self = {};
  ...
  self.updateHtml = function() {
    ...
    if (status.job_state !== runningText) {
      markJobAsErrored();
    }
  };
  ...
  function getStatus() {
    ...
    var ipIndex = $row.data("ip-index");
    if (statusMap[jobName]) {
      return statusMap[jobName][ipIndex];
    } else {
      return null;
    }
  }
  ...
  function setIndex() {
    if (status) {
      $row.find(".index").text(status.index);
    }
  }

  function setColumnValues() {
    var vitals = status.vitals;

    $row.addClass(status.job_state);
    $row.find(".cid").text(status.vm_cid);
    $row.find(".actual-ips").text(status.ips.join(", "));
    $row.find(".az").text(status.az);
    $row.find(".load-avg").text(vitals.load[2]);
    $row.find(".cpu").text(vitals.cpu.user + "%");
    $row.find(".mem").text(vitals.mem.percent + "%");
    $row.find(".swap").text(vitals.swap.percent + "%");
    $row.find(".system-disk").text(vitals.disk.system.percent + "%");
    if (vitals.disk.ephemeral) {
      $row.find(".ephemeral-disk").text(vitals.disk.ephemeral.percent + "%");
    } else {
      $row.find(".ephemeral-disk").text("N/A");
    }
    if (vitals.disk.persistent) {
      $row.find(".persistent-disk").text(vitals.disk.persistent.percent + "%");
    } else {
      $row.find(".persistent-disk").text("N/A");
    }
  }

  function setColumnsToUnknown() {
    $row.addClass(unknownText);
    $row.find(".cid,.actual-ips,.load-avg,.cpu,.mem,.swap,.system-disk,.ephemeral-disk,.persistent-disk").text(unknownText);
  }

  function markJobAsErrored() {
    var $firstJob = $table.find("[data-job-name='" + $row.data("job-name") + "']").first();
...
function InstanceWithError($instanceRow, error) {
  var self = {};
...
$(function() {
  $(".with-status-data").each(function() {
    var $table = $(this);
    var status = StatusData($table);

    status.toggleLoading(true);

    $table.on("action.exit", function(e, data) {
      console.log(JSON.stringify(data.result));
      status.apply(data.result);
    });
  });
});
function bindTextUpdate($el) {
  $el.on("input", ".with-text-update", function () {
    var inputSelector = $(this).data("input-selector");
    var outputSelector = $(this).data("output-selector");
    var text = $(this).find(inputSelector).val();
    $(this).find(outputSelector).text(text);
  });
}
